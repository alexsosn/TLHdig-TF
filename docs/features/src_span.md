<!-- generated by TLHdig-TF featuredocs; do not edit -->
# `src_span`

[Feature reference](0_home.md)

**Module:** provenance

**Kind:** node feature

**Availability:** optional provenance module; not loaded by default.

## Description

byte range of the source element. WARNING: for the 166 repaired documents whose patches change length, this indexes the repaired byte stream, not the file named by src_file. See KNOWN-ISSUES.md.

## Release metadata

- **Value type:** str
- **TF artifact version:** 0.3.0
- **Source version:** 0.3
- **Source DOI:** 10.5281/zenodo.20328284
- **Source license:** CC-BY-4.0
- **Feature license:** CC-BY-4.0
- **Attribution:** Derived from TLHdig 0.3 (Hethitologie-Portal Mainz), CC-BY-4.0. Cite the source dataset, doi:10.5281/zenodo.20328284, not this conversion.

---

Generated from the shipped Text-Fabric header at `tf-provenance/0.3.0/src_span.tf`; corpus feature semantics are checked against `programs/tlhdig/featuremeta.py`.
