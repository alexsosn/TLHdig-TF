...
After the first full-green implementation, independent review identified that Python `A.webLink()` worked but TF browser passage headings still exposed no upstream action because `_sectionLink()` always calls `_noUrl=True`. A second RED regression was then committed requiring:
...
The review-driven RED run had 533 passing tests and exactly one failure: the missing browser source action.
