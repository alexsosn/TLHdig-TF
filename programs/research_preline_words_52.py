...
This script does not change converter behavior. It inventories the repaired production
source, reconciles source-token counts against the current shipped TF graph, and writes
the detailed issue-#52 research report.
"""
...
from tlhdig import TF_VERSION, repair, signs, source, sourcepath
...
EXPECTED_PRELINE_READABLE_WORDS = 15
EXPECTED_PRELINE_READABLE_SIGNS = 22
EXPECTED_SOURCE_SIGNS = 3_365_151
EXPECTED_CURRENT_NON_ANCHOR_SIGNS = 3_365_129
...
def source_inventory():
    ...
    for path in corpus_files():
        ...
        try:
            span_for, _top_words = span_pairing(data, text, spans)
        ...
        for node in text.iter():
            ...
            if first_lb is not None:
                first_lb_desc = ", ".join(f"{k}={v!r}" for k, v in first_lb.attrib.items())
            rows.append(
                {
                    "src_file": src_file,
...
def graph_inventory():
    ...
    api = Fabric(locations=str(tf_dir), silent="deep").load("otype anchor", silent="deep")
    if api is False or api is None:
        raise RuntimeError(f"cannot load tf/{TF_VERSION}")
    graph = Counter()
    anchor = getattr(api.F, "anchor", None)
    for sign in api.F.otype.s("sign"):
        if anchor is not None and anchor.v(sign):
            graph["anchors"] += 1
        else:
...
def write_report(rows, stats, parents, readable_parents, ancestors, graph):
    ...
    problems = []
    expected = {
        "preline_words": EXPECTED_PRELINE_WORDS,
        "preline_readable_words": EXPECTED_PRELINE_READABLE_WORDS,
        "preline_readable_signs": EXPECTED_PRELINE_READABLE_SIGNS,
        "all_source_signs": EXPECTED_SOURCE_SIGNS,
    }
    for key, want in expected.items():
        if stats[key] != want:
            problems.append(f"{key} {stats[key]} != frozen research value {want}")
    if graph["non_anchor_signs"] != EXPECTED_CURRENT_NON_ANCHOR_SIGNS:
        problems.append(
            f"current non-anchor signs {graph['non_anchor_signs']} != frozen pre-fix baseline "
            f"{EXPECTED_CURRENT_NON_ANCHOR_SIGNS}"
        )
    source_minus_tf = stats["all_source_signs"] - graph["non_anchor_signs"]
    if source_minus_tf != stats["preline_readable_signs"]:
        problems.append(
            f"source-minus-TF {source_minus_tf} != pre-line readable signs "
            f"{stats['preline_readable_signs']}"
        )

    lines = [
        "# Research: readable words before the first line boundary",
        "",
        f"Research-only inventory against current TF `{TF_VERSION}` for issue #52.",
        "",
        ...
        f"- current non-anchor TF signs: **{graph['non_anchor_signs']:,}**",
        f"- exact source-minus-TF delta: **{source_minus_tf:,}**",
        f"- current technical anchor signs: **{graph['anchors']:,}**",
        f"- pre-line words carrying markers: **{stats['preline_words_with_markers']:,}**",
        f"- pre-line words carrying notes: **{stats['preline_words_with_notes']:,}**",
        f"- affected documents with no `<lb>`: **{stats['preline_documents_without_lb']:,}**",
        "",
        "## Structural owners",
        "",
        "| parent | all | readable |",
    ...
    lines.extend([
        "",
        "Ancestors: " + ", ".join(f"`{k}` {v:,}" for k, v in ancestors.most_common(12)),
        "",
        "## Complete pre-line inventory",
        "",
        "| source | XML path | bytes | readable | symbols | markers | notes | first line | source word |",
        "| --- | --- | ---: | ---: | --- | --- | ---: | --- | --- |",
    ])
    for row in rows:
        if "error" in row:
            lines.append(f"| `{row['src_file']}` | ERROR |  |  |  |  |  |  | `{row['error']}` |")
            continue
        ...
        lines.append(
            f"| `{row['src_file']}` | `{row['path']}` | `{row['span']}` | {row['readable']} | "
            f"`{symbols}` | `{markers}` | {row['notes']} | `{short(first_line, 80)}` | `{raw}` |"
        )
    lines.extend(["", "## Research gate result", ""])
    lines.append("**FAIL**" if problems else "**PASS** — the 22-sign deficit reconciles exactly.")
