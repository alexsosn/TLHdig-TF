...
def header_structure_problems(root) -> list[str]:
    ...
    even though the vocabulary itself is unchanged. Explicit placement-loss declarations
    are the only exception, and remain reported as current loss rather than represented.
    """
...
def _without_placement_losses(inv: Inventory) -> tuple[Counter, Counter]:
...
def render_report(inv: Inventory) -> str:
    ...
    visible_header, visible_attrs = _without_placement_losses(inv)
    lines = [
    ...
    if inv.header_placement_loss:
        lines += [
            "## Placement-specific known header loss", "",
            "These occurrences use known vocabulary outside the structural paths consumed by the converter.", "",
...
def main() -> int:
    ...
    if failed:
        ...
        if missing_placements:
            print(f"  undeclared known-loss placements ({len(missing_placements)}):")
            for parent, name in missing_placements:
                print(f"    {parent}/{name}")
        if extra_placements:
            print(f"  declared but unobserved known-loss placements ({len(extra_placements)}):")
            for parent, name in extra_placements:
                print(f"    {parent}/{name}")
        if missing_placement_attrs:
            print(f"  undeclared known-loss placement attributes ({len(missing_placement_attrs)}):")
            for parent, name, attr in missing_placement_attrs:
                print(f"    {parent}/{name}@{attr}")
        if extra_placement_attrs:
            print(f"  declared but unobserved known-loss placement attributes ({len(extra_placement_attrs)}):")
