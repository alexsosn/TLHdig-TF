...
def _generated_python_cache(path: str) -> bool:
    """Return true only for ordinary interpreter bytecode generated by imports.

    Certification imports modules before its final protected-tree check, so treating
    normal ``__pycache__`` output as an input would self-invalidate every clean run.
    Keep the exemption deliberately narrower than the repository's ignore rules: an
    arbitrary ignored Python/source/config file under a protected root must still fail.
    """
    p = PurePosixPath(path)
    return "__pycache__" in p.parts and p.suffix in {".pyc", ".pyo"}
...
def _dirty_paths(root: Path) -> list[str]:
    ...
    for args in commands:
        for path in _decode_paths(_run(root, *args)):
            if _protected(path, PROFILE) and not _generated_python_cache(path):
                dirty.add(path)
    return sorted(dirty)
