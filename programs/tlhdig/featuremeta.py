...
DESCRIPTIONS = {
    ...
    "cu_aligned": "how this line's cuneiform was laid out per sign: 0 not aligned, 1 counts matched and were zipped, 2 aligned after absorbing surplus U+2592 placeholders into a recorded lacuna, 3 aligned after expanding a compound logogram, 4 aligned using the derived numeral rule. Higher numbers are weaker evidence, so a query wanting only the safest material filters cu_aligned=1. Measured against the learned reading->sign table, level 1 assignments contradict it in 0.01% of cases, level 2 in 0.96% and level 3 in 0.15%; see reports/alignment.md. Level 4 no longer occurs -- the numerals are now learned spellings and reach level 3 -- but the value is kept so an older dataset still reads correctly.",
    "cu_method": "which alignment mechanisms actually ran on this line, space-separated: zip, damage, compound, numeral. `cu_aligned` cannot say this on its own -- a compound line that also absorbed damage is level 3 and used two mechanisms.",
    "cu_undecided": "how many signs on this line the alignment left unassigned because two readings of the line explained that position equally well. Present only when non-zero.",
