...
**Current TF version: `0.5.0` — integration prototype. Do not use it as the sole basis for
research conclusions yet.**

The generated dataset is committed in [`tf/0.5.0/`](tf/0.5.0), the Text-Fabric app
...
The build currently exposes 120 node features and 14 edge features in addition to the
...
tf/0.5.0/               current generated Text-Fabric dataset
tf-provenance/0.5.0/    current optional source-provenance module
```

Two version numbers are intentionally separate: `sourceVersion = 0.3` identifies the
upstream TLHdig release, while `tfVersion = 0.5.0` identifies this conversion model and
build.
